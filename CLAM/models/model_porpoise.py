...
class PorpoiseMMF(nn.Module):
    def __init__(self, 
        ...
        if self.fusion is not None:
            ...
            fc_omic = [Block(dim1=self.omic_input_dim, dim2=hidden[0])]
    ...
    def forward(self, h):
        ...
        x_omic = h[1].unsqueeze(0) # torch.Size([1, 20970])
        ...
        logits  = self.classifier_mm(h_mm) # logits needs to be a [B x 2] vector      
        assert len(logits.shape) == 2 and logits.shape[1] == self.n_classes

        # instance_eval = False # temporary.
        # if instance_eval:
